...
import { 
    games, statuses, languages, users, controllers, platforms, genres, tags,
...
gameRoutes.post('/new', authorizeRole(['developer', 'admin']), async (req: Request, res: Response) => {
    try {
        const {
        ...
        if (controllerIds?.length) {
            await db.insert(gameControllers).values(
                controllerIds.map((id: number) => ({ GameId: gameId, ControllerId: id }))
            );
        }

        if (platformIds?.length) {
            await db.insert(gamePlatforms).values(
                platformIds.map((id: number) => ({ GameId: gameId, PlatformId: id }))
            );
        }

        if (genreIds?.length) {
            await db.insert(gameGenres).values(
                genreIds.map((id: number) => ({ GameId: gameId, GenreId: id }))
            );
        }

        if (tagIds?.length) {
            await db.insert(gameTags).values(
                tagIds.map((id: number) => ({ GameId: gameId, TagId: id }))
            );
        }
...
gameRoutes.get('/id/:gameId', async (req: Request, res: Response) => {
    try {
...
gameRoutes.get('/search', async (req: Request, res: Response) => {
    ...
    try {
        ...
        const gamesWithRelations = await Promise.all(
            foundGames.map(game => getGameWithRelations(game.id))
        );
...
gameRoutes.get('/price/:min/:max', async (req: Request, res: Response) => {
    try {
...
// GET /sequence/date - Récupérer les jeux triés par date de création
gameRoutes.get('/sequence/date', async (req: Request, res: Response) => {
    try {
        ...
        const gamesWithRelations = await Promise.all(
            gamesDate.map(game => getGameWithRelations(game.id))
        );
...
gameRoutes.delete('/delete/:id', authorizeRole(['admin', 'developer', 'superadmin']), async (req: Request, res: Response) => {
...
gameRoutes.put('/update/:id', authorizeRole(['admin', 'developer', 'superadmin']), async (req: Request, res: Response) => {
    const gameId = parseInt(req.params.id);
...
gameRoutes.post('/associate-categories', async (req: Request, res: Response) => {
    ...
    try {
        ...
        const controllerIds = Array.isArray(ControllerId) ? ControllerId.map((id: any) => parseInt(id)) : [parseInt(ControllerId)];
        const platformIds = Array.isArray(PlatformId) ? PlatformId.map((id: any) => parseInt(id)) : [parseInt(PlatformId)];
        const tagIds = Array.isArray(TagId) ? TagId.map((id: any) => parseInt(id)) : TagId ? [parseInt(TagId)] : [];
        const genreIds = Array.isArray(GenreId) ? GenreId.map((id: any) => parseInt(id)) : GenreId ? [parseInt(GenreId)] : [];
...
gameRoutes.get('/by-user/:userId', authorizeRole(['developer']), async (req: Request, res: Response) => {
    try {
        ...
        const gamesWithRelations = await Promise.all(
            userGames.map(game => getGameWithRelations(game.id))
        );
...
gameRoutes.get('/last-updated', async (req: Request, res: Response) => {
    try {
        ...
        const gamesWithRelations = await Promise.all(
            gamesUpdated.map(game => getGameWithRelations(game.id))
        );
...
gameRoutes.get('/check-relations', async (req: Request, res: Response) => {
    try {
        ...
        const gamesStatus = await Promise.all(
...
async function getGameWithRelations(gameId: number) {
    const [game] = await db.select().from(games).where(eq(games.id, gameId)).limit(1);
