...
commentRoutes.get('/game/:gameId', async (req: Request, res: Response) => {
    try {
...
commentRoutes.post('/create', verifyToken, async (req: Request, res: Response) => {
    try {
        ...
        const existingComment = await db.select()
            ...
            .where(and(
                ...
                eq(comments.GameId, parseInt(GameId))
            ))
            .limit(1);
...
commentRoutes.put('/update/:commentId', verifyToken, async (req: Request, res: Response) => {
    try {
        const userId = req.user?.id ? parseInt(req.user.id) : null;
...
commentRoutes.delete('/delete/:commentId', verifyToken, async (req: Request, res: Response) => {
    try {
        const userId = req.user?.id ? parseInt(req.user.id) : null;
        const commentId = parseInt(req.params.commentId);
...
commentRoutes.get('/user/:userId', async (req: Request, res: Response) => {
    try {
