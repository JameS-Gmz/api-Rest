...
import { DataTypes, where, Op } from "sequelize";
...
UserRoute.get('/library/allgames', verifyToken, async (req: any, res) => {
    try {
        ...
        const games = userWithGames.get('libraryGames') as any[];
...
UserRoute.post('/library/games/:gameId', verifyToken, async (req: any, res) => {
    try {
...
UserRoute.delete('/library/games/:gameId', verifyToken, async (req: any, res) => {
    try {
...
UserRoute.get('/library/games/:gameId/check', verifyToken, async (req: Request, res: Response) => {
    try {
        const userId = req.user?.id;
        const gameId = req.params.gameId;
